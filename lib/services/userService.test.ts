...
describe('profileToUser', () => {
    ...
    describe('Edge cases', () => {
        ...
        it('should handle profile with ISO 8601 timestamp with milliseconds', () => {
            const profile = { ...validProfile, created_at: '2024-01-15T10:30:00.123Z' };
            const result = profileToUser(profile);
            expect(result.success).toBe(true);
            if (result.success) {
...
import { classifySupabaseError, createErrorResponse, createSuccessResponse } from './userService';
...
describe('classifySupabaseError', () => {
    ...
    describe('Constraint errors', () => {
        ...
        it('should include original message in details', () => {
            const error = new Error('Unique constraint violation');
            const result = classifySupabaseError(error);
            expect(result.details?.originalMessage).toBe('Unique constraint violation');
        });
    });
    ...
    describe('Error classification priority', () => {
        it('should prioritize network errors over other errors', () => {
            const error = { code: 'ECONNREFUSED', message: 'Connection refused', status: 401 };
            const result = classifySupabaseError(error);
            expect(result.code).toBe('NETWORK_ERROR');
        });
...
describe('createErrorResponse', () => {
    describe('Basic error response creation', () => {
        it('should create error response with code and message', () => {
            const response = createErrorResponse('NETWORK_ERROR', 'Connection failed');
        ...
        it('should create error response with all error codes', () => {
            const codes: Array<'NETWORK_ERROR' | 'AUTH_ERROR' | 'PERMISSION_ERROR' | 'VALIDATION_ERROR' | 'CONSTRAINT_ERROR' | 'NOT_FOUND_ERROR' | 'UNKNOWN_ERROR'> = [
                'NETWORK_ERROR',
                'AUTH_ERROR',
                'PERMISSION_ERROR',
                'VALIDATION_ERROR',
        ...
        it('should create error response with details', () => {
            ...
            const response = createErrorResponse('NETWORK_ERROR', 'Connection failed', details);
    ...
    describe('Error response structure', () => {
        ...
        it('should have error property with code, message, and details', () => {
            const response = createErrorResponse('NETWORK_ERROR', 'Connection failed', { test: 'value' });
    ...
    describe('Details handling', () => {
        it('should preserve details object structure', () => {
            const details = { field: 'email', reason: 'invalid format', value: 'test@' };
        ...
        it('should handle nested details object', () => {
            ...
            const response = createErrorResponse('UNKNOWN_ERROR', 'Error occurred', details);
            expect(response.error.details?.error).toEqual({ nested: { value: 'test' } });
        });
...
import { calculatePaginationMetadata, validatePaginationParams, PaginationMetadata } from './userService';
...
describe('calculatePaginationMetadata', () => {
    describe('Valid pagination calculations', () => {
        ...
        it('should calculate hasPreviousPage correctly', () => {
            // Page 1 (first page)
            expect(calculatePaginationMetadata(1, 10, 25).hasPreviousPage).toBe(false);
            // Page 2 (middle page)
        ...
        it('should search users by display_name', async () => {
            ...
            if (result.success) {
                ...
                result.data.data.forEach(user => {
                    ...
                    expect(
                        displayName.includes('john') || email.includes('john')
                    ).toBe(true);
                });
            }
        });
