...
describe('validateUser - Property-Based Tests', () => {
    ...
    const roleArbitrary = () =>
        fc.constantFrom('member', 'author', 'admin');
    ...
    describe('Property 7: User data validation enforces schema constraints', () => {
        ...
        it('should reject profiles with invalid role values', () => {
            fc.assert(
                fc.property(
                    validUserArbitrary(),
                    fc.stringMatching(/^[a-z]+$/).filter(role => !['member', 'author', 'admin'].includes(role)),
                    (user, invalidRole) => {
        ...
        it('should reject profiles with invalid status values', () => {
            fc.assert(
                fc.property(
                    validUserArbitrary(),
                    fc.stringMatching(/^[a-z]+$/).filter(status => !['active', 'suspended'].includes(status)),
                    (user, invalidStatus) => {
        ...
        it('should reject profiles with invalid id format', () => {
            fc.assert(
                fc.property(
                    validUserArbitrary(),
                    fc.stringMatching(/^[a-z0-9]+$/).filter(id => id.length > 0 && !id.includes('-')),
                    (user, invalidId) => {
            ...
            fc.assert(
                fc.property(
                    ...
                    (user, invalidTimestamp) => {
                        const result = validateUser({ ...user, created_at: invalidTimestamp });
                        return (
                            result.success === false &&
                            result.error.code === 'VALIDATION_ERROR' &&
                            result.error.details?.field === 'created_at'
