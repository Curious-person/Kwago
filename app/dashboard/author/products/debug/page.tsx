...
export default function ProductsDebugPage() {
  ...
  const runDiagnostics = async () => {
    ...
    if (tableError) {
      ...
      addResult(
        "   Please run: database/products_schema.sql in Supabase SQL Editor\n",
      );
      setIsRunning(false);
      return;
    }
    ...
    addResult(
      `   Attempting to insert: ${JSON.stringify(testProduct, null, 2)}`,
    );
    ...
    if (deleteError) {
      ...
      addResult(
        `   Please manually delete product ID: ${insertedProduct.id}\n`,
      );
      setIsRunning(false);
      return;
    }
      ...
      <div className="mt-8 p-6 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <div className="space-y-4 text-sm text-blue-800">
          <div>
            ...
            <p className="mt-1">
              The products table doesn't exist. Run{" "}
          ...
          <div>
            ...
            <p className="mt-1">
              Row Level Security policies are not configured correctly. Make
              sure the schema file was run completely.
            </p>
          </div>
          <div>
            <strong>❌ Authentication failed:</strong>
            <p className="mt-1">
              You're not logged in or your session expired. Log in again and
              retry.
            </p>
          </div>
        </div>
