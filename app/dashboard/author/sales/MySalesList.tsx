...
export function MySalesList({ initialSales }: MySalesListProps) {
  ...
  const [editingStatusSale, setEditingStatusSale] =
    useState<ProductSalesData | null>(null);
  const [isStatusDialogOpen, setIsStatusDialogOpen] = useState(false);

  const [restockingSale, setRestockingSale] = useState<ProductSalesData | null>(
    null,
  );
  ...
  const saveStatusUpdate = (
    saleToUpdate: ProductSalesData,
    newStatus: ProductSalesStatus,
  ) => {
    setSales((prev) =>
      prev.map((s) =>
    ...
    setSales((prev) =>
      prev.map((s) =>
        s.productId === saleToUpdate.productId
          ? {
              ...s,
              stockRemaining: s.stockRemaining + additionalStock,
              status:
                s.stockRemaining + additionalStock > 0 &&
  ...
  const filteredSales = sales.filter(
    (s) =>
      ...
      s.product.category_names?.some((name) =>
  ...
  const totalUnitsSold = sales.reduce(
    (acc, sale) => acc + sale.totalUnitsSold,
    0,
  );
        ...
        <div className="flex flex-col items-center justify-center py-32 text-center">
          ...
          <p className="text-zinc-500 max-w-xs">
            {searchQuery
              ...
              : "You haven't tracked any sales yet."}
          </p>
        </div>
      ) : viewMode === "grid" ? (
          ...
          <Table>
            <TableHeader className="bg-zinc-50/50">
              <TableRow className="hover:bg-transparent border-zinc-100">
                <TableHead className="w-[80px] text-zinc-400 font-bold uppercase tracking-widest text-[10px]">
                  Photo
                </TableHead>
                <TableHead className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">
                  Product Details
                </TableHead>
                <TableHead className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">
                  Stats
                ...
                <TableHead className="text-right text-zinc-400 font-bold uppercase tracking-widest text-[10px]">
                  Actions
                </TableHead>
              </TableRow>
            </TableHeader>
                ...
                >
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-0.5">
                      ...
                      <span className="text-xs text-zinc-400">
                        {sale.product.category_names &&
                        sale.product.category_names.length > 0
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-0.5">
                      ...
                      <span className="text-xs text-zinc-400">
                        {sale.totalUnitsSold} units sold
                      </span>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent
                        align="end"
                        className="rounded-2xl w-48 p-2 border-zinc-100 shadow-sm"
                      >
                        <DropdownMenuGroup>
