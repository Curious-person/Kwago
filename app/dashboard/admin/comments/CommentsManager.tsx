...
const MOCK_DATA: Post[] = [
  {
    ...
    flaggedComments: 2,
    comments: [
      { id: "c1", author: "John Doe", text: "Great insights on minimalism! I really love how you explained the use of space.", status: "approved", date: "2 hours ago" },
      { id: "c2", author: "Jane Smith", text: "I disagree with the point about shadows. Sometimes they add necessary depth.", status: "flagged", date: "5 hours ago", reason: "Potential spam" },
      { id: "c3", author: "Bob Wilson", text: "Buy cheap crypto here! Best rates guaranteed at crypto-scam.com", status: "flagged", date: "1 day ago", reason: "Spam/Advertising" },
    ]
  },
  {
    id: "p2",
    ...
    flaggedComments: 0,
    comments: [
      { id: "c4", author: "Alice Brown", text: "This is exactly what the industry needs right now.", status: "approved", date: "3 hours ago" },
    ]
  },
  {
    id: "p3",
...
export function CommentsManager() {
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500">Managing {selectedPost.comments.length} comments</p>
          </div>
        </div>
          ...
          columns={[
            {
              ...
              cell: (row) => (
                <div className="flex flex-col gap-0.5">
                  ...
                  <div className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest mt-1">{row.date}</div>
                </div>
              )
            },
            {
              ...
              cell: (row) => (
                <div className="max-w-md">
                  <p className="text-zinc-500 text-sm leading-relaxed">{row.text}</p>
                  {row.status === 'flagged' && row.reason && (
                    <div className="mt-2 flex items-start gap-2 p-3 bg-red-50 rounded-2xl border border-red-100">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-red-700 uppercase tracking-wider">Reason</p>
                        <p className="text-xs text-red-600 mt-0.5">{row.reason}</p>
                      </div>
                    </div>
                  )}
                  {flaggingCommentId === row.id && (
                    <div className="mt-4 flex flex-col gap-3 p-4 bg-zinc-50 rounded-2xl border border-zinc-200">
                      <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Reason for flagging</label>
                      <input 
                        type="text" 
            ...
            },
            {
              ...
              filterable: true,
              cell: (row) => (
                <Badge variant={row.status === 'flagged' ? 'secondary' : 'active'} className="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1">
                  {row.status}
                </Badge>
              )
            },
  ...
  const postColumns: ColumnDef<Post>[] = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex flex-col gap-0.5">
          <div className="font-bold text-zinc-900 group-hover:text-[#0066FF] transition-colors">{row.title}</div>
    ...
    },
    {
      ...
      cell: (row) => <span className="font-medium text-zinc-500 text-sm">{row.totalComments}</span>
    },
    {
      header: 'Flagged',
      ...
      cell: (row) => (
        <div className={cn(
          ...
          row.flaggedComments > 0 ? "bg-red-50 text-red-500" : "bg-zinc-50 text-zinc-400"
        )}>
          {row.flaggedComments}
        </div>
      )
