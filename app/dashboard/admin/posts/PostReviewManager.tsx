...
export function PostReviewManager() {
  ...
  const handleConfirmReject = async () => {
    ...
    const result = await adminReviewPost(rejectTarget.id, 'reject', rejectRemarks);
    if (result.success) {
      await fetchPosts();
  ...
  const columns = React.useMemo<ColumnDef<Post>[]>(() => [
    ...
    },
    {
      ...
      filterable: true,
      cell: (row) => {
        const cfg = STATUS_BADGE[row.status as PostStatus] ?? STATUS_BADGE.Draft;
        return (
          <Badge
            variant="outline"
            ...
            <DialogDescription className="mt-2 space-y-4 block">
              ...
              <span className="flex gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 mt-3">
                ...
                <span className="text-sm text-blue-800 leading-relaxed">
                  <span className="font-semibold">Heads up:</span> This post will
                  become publicly visible on the homepage alongside other approved
                  articles. You can unpublish it at any time from this dashboard.
                </span>
              </span>
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  {(() => {
                    const cfg = STATUS_BADGE[selectedPost.status as PostStatus] ?? STATUS_BADGE.Draft;
                    return (
                      <Badge
                        variant="outline"
