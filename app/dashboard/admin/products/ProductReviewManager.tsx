...
export function ProductReviewManager() {
  ...
  const [actionToConfirm, setActionToConfirm] = useState<{ id: string, action: 'approve' | 'reject', name: string } | null>(null);
  ...
  const handleReviewAction = (id: string, action: 'approve' | 'reject', name: string) => {
  ...
  const confirmReviewAction = async () => {
    ...
    const result = await adminReviewProduct(actionToConfirm.id, actionToConfirm.action);
    if (result.success) {
          ...
          {row.status === 'ai-approved' && (
            <>
              ...
              <Button 
                ...
                onClick={() => handleReviewAction(row.id, 'approve', row.name)}
              >
        ...
        <DialogContent className="sm:max-w-md rounded-3xl p-8 border-zinc-100 shadow-none">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold text-zinc-900">
              {actionToConfirm?.action === 'approve' ? 'Approve Product' : 'Reject Product'}
            </DialogTitle>
            <DialogDescription className="text-sm text-zinc-500 mt-2 leading-relaxed">
              Are you sure you want to {actionToConfirm?.action} <span className="font-semibold text-zinc-900">"{actionToConfirm?.name}"</span>? 
              {actionToConfirm?.action === 'approve' ? ' This will publish the product to the marketplace.' : ' This will send it back to the author.'}
            </DialogDescription>
          </DialogHeader>
