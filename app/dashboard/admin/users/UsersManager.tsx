...
function StatusBadge({ status }: { status: UserStatus }) {
  return status === "active" ? (
    <Badge
      variant="active"
      className="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 pointer-events-none"
    >
      Active
    </Badge>
  ) : (
    <Badge
      variant="secondary"
      className="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 text-zinc-500 pointer-events-none"
    >
      Suspended
    </Badge>
  );
...
export function UsersManager() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      if (result.success) {
        ...
        console.error(
          "[UsersManager] Error fetching users:",
          (result as any).error,
        );
      }
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((u) => {
      const matchesTab =
        activeTab === "members" ? u.role === "member" : u.role === "author";
      const matchesSearch =
        searchQuery === "" ||
  ...
  const members = useMemo(
    () => filteredUsers.filter((u) => u.role === "member"),
    [filteredUsers],
  );
  const authors = useMemo(
    () => filteredUsers.filter((u) => u.role === "author"),
    [filteredUsers],
  );
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          `[UsersManager] Action confirmed: ${type} → ${user.display_name}`,
        );
      } else {
        setError(result as ErrorResponse);
  ...
  const userInfoCell = useCallback(
    (row: User) => (
    ...
    () => [
      {
        header: "User",
        filterable: true,
        filterValue: (row) => `${row.display_name} ${row.email}`,
        cell: userInfoCell,
      },
      {
        header: "Joined",
      ...
      { header: "Status", cell: (row) => <StatusBadge status={row.status} /> },
      {
        ...
        cell: (row) => (
          <div className="flex items-center justify-end gap-2">
            ...
            >
              <ShieldMinus className="w-3 h-3 mr-1.5" />
              Demote
            </Button>
          </div>
        ),
          ...
          {actionConfig && pendingAction && (
            <>
              ...
              <div className="flex items-center gap-3 px-4 py-3 bg-zinc-50 rounded-2xl border border-zinc-100 mt-2">
                ...
                <div>
                  <p className="text-sm font-bold text-zinc-900">
                    {pendingAction.user.display_name}
                  </p>
              ...
              <div className="flex gap-3 w-full mt-8">
                <DialogPrimitive.Close className="flex-1 inline-flex items-center justify-center rounded-full border border-zinc-200 bg-white px-6 py-2.5 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-50">
                  Cancel
