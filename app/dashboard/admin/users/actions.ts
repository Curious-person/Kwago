...
import { getUsers, updateUserRole, updateUserStatus } from '@/lib/services/userService';
 ...
 */
export async function promoteUserAction(userId: string): Promise<ServiceResponse<User>> {
    try {
        // Check admin authorization
        const isAdmin = await checkAdminRole();
        if (!isAdmin) {
 ...
 * @returns ServiceResponse<User> - Updated user object or error
 */
export async function demoteUserAction(userId: string): Promise<ServiceResponse<User>> {
    try {
        // Check admin authorization
        const isAdmin = await checkAdminRole();
        if (!isAdmin) {
 ...
 * @returns ServiceResponse<User> - Updated user object or error
 */
export async function suspendUserAction(userId: string): Promise<ServiceResponse<User>> {
    try {
        // Check admin authorization
        const isAdmin = await checkAdminRole();
        if (!isAdmin) {
 ...
 * @returns ServiceResponse<User> - Updated user object or error
 */
export async function reactivateUserAction(userId: string): Promise<ServiceResponse<User>> {
    try {
        // Check admin authorization
        const isAdmin = await checkAdminRole();
        if (!isAdmin) {
