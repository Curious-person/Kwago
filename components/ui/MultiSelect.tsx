...
export function MultiSelect({
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            ...
            />
            <PopoverContent className="w-full p-0 border-zinc-200 rounded-3xl shadow-none mt-2 overflow-hidden bg-white" align="start">
                <Command className="rounded-3xl border-none">
                    ...
                    />
                    <CommandList>
                        {filteredOptions.length === 0 && <CommandEmpty>No categories found.</CommandEmpty>}
                        <CommandGroup className="p-2">
                            {filteredOptions.map((option) => (
                                ...
                                >
                                    <span className={cn(
                                        "text-sm font-medium",
                                        selected.includes(option.value) ? "text-zinc-900" : "text-zinc-600"
                                    )}>
                                        {option.label}
                                    </span>
                                    {selected.includes(option.value) && (
