...
export const CategoryCard: React.FC<CategoryCardProps> = React.memo(
    ({ category, onEdit, onDelete, onViewProducts }) => {
        ...
        return (
            <div className="rounded-3xl p-6 border border-zinc-100 bg-white hover:border-zinc-200 transition-colors">
                {/* Header with title and product count */}
                <div className="mb-6 flex items-start justify-between gap-4">
                    <div className="flex-1">
                        <h3 className="text-lg font-bold text-zinc-900 mb-2">{category.name}</h3>
                        {category.description && (
                            <p className="text-sm text-zinc-600 line-clamp-2">{category.description}</p>
                        )}
                    </div>
                    <Badge variant="active" className="whitespace-nowrap">
                    ...
                    {displayImages.length > 0 ? (
                        <div className="flex gap-3 items-center">
                            {displayImages.map((image, index) => (
                                <div
                                    key={index}
                                    className="relative w-16 h-16 rounded-xl overflow-hidden bg-zinc-100 flex-shrink-0"
                                >
                                    <Image
                                        src={image}
                                        alt={`Product ${index + 1}`}
                            ...
                            {moreCount > 0 && (
                                <div className="w-16 h-16 rounded-xl bg-zinc-100 flex items-center justify-center flex-shrink-0">
                                    <span className="text-sm font-bold text-zinc-600">+{moreCount}</span>
                                </div>
                            )}
                        </div>
                    ) : (
