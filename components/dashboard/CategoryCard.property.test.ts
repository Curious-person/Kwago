...
describe("CategoryCard - Property-Based Tests", () => {
   ...
   */
  const productCountArbitrary = () => fc.integer({ min: 0, max: 1000 });
   ...
   * Implements the same logic as the CategoryCard component
   */
  const shouldShowNoProductsPlaceholder = (
    productCount: number,
  ...
  describe("Property 3: Product Count Badge Display", () => {
    ...
    it("should always include the correct count number", () => {
      fc.assert(
        fc.property(productCountArbitrary(), (count) => {
          const text = getExpectedProductCountText(count);
  ...
  describe("Property 4: Product Image Preview", () => {
    it("should display up to 3 product images", () => {
      fc.assert(
        fc.property(productImagesArbitrary(), (images) => {
    ...
    it('should display "+N more" indicator when product_count > 3', () => {
      fc.assert(
        fc.property(fc.integer({ min: 4, max: 1000 }), (count) => {
      ...
      fc.assert(
        fc.property(
          ...
          (count, images) => {
            ...
            const isValidMoreCount =
              count <= 3 ? moreCount === 0 : moreCount > 0;

            // No products placeholder only when count is 0 and no images
            const isValidNoProducts =
              noProducts === (count === 0 && images.length === 0);
    ...
    it("should handle realistic category scenarios", () => {
      const scenarios = [
        ...
        },
        {
          count: 5,
          images: ["img1", "img2", "img3"],
          description: "5 products, 3 images",
        },
        {
          count: 10,
          images: ["img1", "img2", "img3"],
          description: "10 products, 3 images",
        },
        {
          count: 100,
          images: ["img1", "img2", "img3"],
          description: "100 products, 3 images",
        },
      ];
