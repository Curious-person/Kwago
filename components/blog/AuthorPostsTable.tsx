...
export default function AuthorPostsTable({
  initialPosts,
}: AuthorPostsTableProps) {
  ...
  const columns = React.useMemo<ColumnDef<Post>[]>(
    () => [
      ...
      },
      {
        ...
        accessorKey: "date",
        cell: (row) => (
          <span className="text-zinc-500 text-sm font-medium">{row.date}</span>
        ),
      },
      {
        header: "Status",
        ...
        cell: (row) => {
          ...
          if (s === "published") {
            return (
              <Badge
                variant="default"
                className="font-bold text-[10px] uppercase tracking-widest rounded-full px-3 py-1 bg-green-500 text-white border-none"
              >
                Published
              </Badge>
            );
          ...
          if (s === "ai_rejected" || s === "ai-declined" || s === "rejected") {
            return (
              <Badge
                variant="outline"
                className="font-bold text-[10px] uppercase tracking-widest rounded-full px-3 py-1 text-red-600 border-red-200 bg-red-50"
              >
                AI Rejected
              </Badge>
            );
          ...
          if (s === "ai_approved" || s === "ai-approved") {
            return (
              <Badge
                variant="outline"
                className="font-bold text-[10px] uppercase tracking-widest rounded-full px-3 py-1 text-blue-600 border-blue-200 bg-blue-50"
              >
                AI Approved
              </Badge>
            );
          ...
          if (
            ...
            return (
              <Badge
                variant="outline"
                className="font-bold text-[10px] uppercase tracking-widest rounded-full px-3 py-1 text-amber-600 border-amber-200 bg-amber-50"
              >
                Pending Admin
              </Badge>
            );
            ...
            <DropdownMenuContent align="end" className="rounded-2xl w-48 p-2">
              <DropdownMenuGroup>
                ...
                <DropdownMenuItem
                  className="gap-2 px-3 py-2 rounded-xl cursor-pointer"
                  onClick={() =>
                    router.push(`/dashboard/author/posts/${row.id}/edit`)
                  }
                >
                  ...
                  <Badge
                    ...
                    className={`ml-auto text-[10px] font-bold uppercase tracking-wider rounded-full px-2.5 py-0.5 ${
                      selectedPost.status === "Published"
                        ...
                        : ["ai_rejected", "ai-declined", "rejected"].includes(
                              selectedPost.status?.toLowerCase() || "",
                            )
                          ? "bg-red-50 text-red-600 border border-red-200"
                          : ["ai_approved", "ai-approved"].includes(
                                selectedPost.status?.toLowerCase() || "",
                              )
