...
export default function InteractiveArticle({ post }: InteractiveArticleProps) {
  ...
  const [translatedBlocks, setTranslatedBlocks] = useState<ContentBlock[]>(
    post.blocks,
  );
  ...
  const articleContentNode = useMemo(() => {
    ...
    return (
      <div
        className={`prose prose-zinc prose-lg max-w-none text-zinc-600 leading-relaxed transition-opacity duration-300 ${isTranslating ? "opacity-40 pointer-events-none" : "opacity-100"}`}
      >
          ...
          switch (block.type) {
            case "headline":
              return (
                <h2
                  key={block.id}
                  className="text-2xl font-bold text-zinc-900 mt-12 mb-6"
                >
            ...
            case "subtitle":
              return (
                <p
                  key={block.id}
                  className="text-xl text-zinc-900 font-medium mb-8 leading-relaxed"
                >
                  {t(block.content, { defaultValue: block.content })}
                </p>
              );
            case "quote":
              return (
                <blockquote
                  key={block.id}
                  className="border-l-4 border-[#0066FF] pl-6 my-10 italic text-xl text-zinc-900 font-medium bg-zinc-50/50 py-1 pr-4"
                >
                  {t(block.content, { defaultValue: block.content })}
                </blockquote>
              );
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mb-10">
          ...
          <div className="flex items-center gap-3 px-5 py-2 rounded-full border border-zinc-100 bg-zinc-50 text-zinc-500 text-xs font-semibold">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={
                  isEnglish
                    ? "text-zinc-900 select-none font-bold"
                    : "text-zinc-400 hover:text-zinc-900 cursor-pointer transition-colors select-none"
                }
              >
                EN
              </button>
          ...
          {!isSpeakingOrPaused ? (
            ...
            >
              <Volume2
                size={14}
                className="text-zinc-400 group-hover:text-[#0066FF] transition-colors"
              />
            ...
            <div className="flex items-center gap-2 transition-all duration-300">
              ...
              {speechStatus === "started" && (
                <div
                  className="flex items-center gap-2 ml-2 pl-2 border-l border-zinc-100"
                  role="status"
                >
