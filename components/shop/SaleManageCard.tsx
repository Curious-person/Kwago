...
export function SaleManageCard({
  saleData,
  onUpdateStatus,
  onRestock,
}: SaleManageCardProps) {
  ...
  const isLowStock =
    saleData.stockRemaining <= 5 && saleData.status !== "Out of Stock";
  ...
  return (
    <div className="group relative flex flex-col bg-white rounded-3xl p-4 border border-zinc-200 shadow-none hover:border-zinc-300 transition-all duration-300">
      ...
      <div className="flex gap-4 mb-4">
        ...
        <div>
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-[160px] rounded-2xl border-zinc-100 shadow-sm"
            >
              <DropdownMenuItem
      ...
      <div className="grid grid-cols-2 gap-2 mb-4 bg-zinc-50 rounded-2xl p-3">
        <div>
          ...
          <p className="font-semibold text-zinc-900">
            ${saleData.totalRevenue.toLocaleString()}
          </p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-widest text-zinc-400 font-medium">
      ...
      <div className="mt-auto flex items-center justify-between pt-2">
        <span
          className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(saleData.status)}`}
        >
          {saleData.status}
        </span>
